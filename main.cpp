...
namespace V1 {
	BOOL matchesW(LPCWSTR string1, LPCWSTR string2) {
		...
		szStr1 = lstrlenW(string1),	szStr2 = lstrlenW(string2);
		if (szStr1 > szStr2 && szStr1 < szStr2) return FALSE;
		for (int i = 0; i < szStr2; i++) {
			if (string1[i] == string2[i]) szStringCount++;
		}
		if (szStringCount < szStr2) return FALSE;
		return TRUE;
	}

	BOOL matchesA(LPCSTR string1, LPCSTR string2) {
		INT szStr1, szStr2, szStringCount = 0;
		szStr1 = lstrlenA(string1), szStr2 = lstrlenA(string2);
		if (szStr1 > szStr2 && szStr1 < szStr2) return FALSE;
		for (int i = 0; i < szStr2; i++) {
			if (string1[i] == string2[i]) szStringCount++;
		}
